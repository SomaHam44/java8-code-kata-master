...
public class Exercise8Test extends ClassicOnlineStore {
    ...
    public void itemsNotOnSale() {
        ...
        Set<String> itemSetNotOnSale = customerStream.flatMap(customer -> customer.getWantToBuy().stream().map(item -> item.getName())
                .filter(i -> itemListOnSale.stream().noneMatch(i::equals))).collect(Collectors.toSet());
